<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8" />
<link rel="stylesheet" href="../css/banner.css">
<link rel="stylesheet" href="../css/common.css">
<link rel="stylesheet" href="../css/index.css">
<link rel="stylesheet" href="../css/reset.css">
</head>
<body>
    <div class="banner pc_area">
        <a href="forbeginners.php">
            <div class=banner_h1>
                初心者向け
            </div>
        </a>
        <div class=banner_link>
            <a href="kihonsousa.php">
                基本操作
            </a>
        </div>
        <div class=banner_link>基礎知識</div>
        <div class=banner_link>武器について</div>
        <div class=banner_link>レジェンドについて</div>
        <a href="bord.php">
            <div class=banner_h1>
                掲示板
            </div>
        </a>
        <div class=banner_link>武器掲示板</div>
        <div class=banner_link>キャラ掲示板</div>
        <div class=banner_link>立ち回り掲示板</div>
        <a href="categoriedmovies.php">
        <div class=banner_h1>
            カテゴリ別動画
        </div>
        </a>
        <div class=banner_link>上達法について</div>
        <div class=banner_link>レジェンド解説</div>
        <div class=banner_link>立ち回り解説</div>
        <div class=banner_link>武器解説</div>
    </div>
</body>
</html>